...
def get_svr_score(pipeline, X_test, y_test):
    ...
    return rf.score(X_test_transformed, y_test)
    
def plot_confusion_matrix(cm, title='rf Confusion matrix', cmap=plt.get_cmap('Blues')):
...
svr_score = get_svr_score(pipeline, X_test, y_test)

print(cm)
print('Accuracy: {}'.format(round(svr_score*100, 4)))
